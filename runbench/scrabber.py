...
class LogScrabber(object):
    ...
    def __init_matchers(self):
        ...
        btor_time_regex = r'^user\s*(?P<val>[0-9][0-9]?)m(?P<seconds>[0-5]?[0-9]\.\d*)s'
        # btor_time_regex = r'^user\s*(?P<val>[0-9][0-9]?m[0-5]?[0-9]\.\d*s)'
        btor_time = ReMatch(regex=btor_time_regex, field='btor_time', seconds=True)
